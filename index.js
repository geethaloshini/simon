...
document.addEventListener("keydown", async function(event) {
    if (event.key === "Enter" && document.getElementById("game").style.display==="block") {
        document.querySelector("#game p").style.display = "none";
            let start = new Audio('https://github.com/user/project.git/main/game-start.mp3');
            start.play();
            await delay(1000);
            startGame()
...
const red=new Audio("https://github.com/user/project.git/main/red.mp3");
const blue=new Audio("https://github.com/user/project.git/main/blue.mp3");
const yellow=new Audio("https://github.com/user/project.git/main/yellow.mp3");
const green=new Audio("https://github.com/user/project.git/main/green.mp3");
    ...
    if (user_generated_pattern[level] === computer_generated_pattern[level]) {
        if (user_generated_pattern.length === computer_generated_pattern.length) {
            if (user_generated_pattern.length === 10) {
                ...
                let win = new Audio('https://github.com/user/project.git/main/win.mp3');
                win.play();
                await delay(4000);
                startGame();
            } else {
        ...
        let over = new Audio('https://github.com/geethaloshini/simon.git/main/game-over.mp3');
        over.play();
        await delay(4000);
           startGame();
    };
